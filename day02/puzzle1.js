...
module.exports.getSolution = () => {
  ...
  for (present of input) {
    ...
    for (let dimension = 0; dimension < present.length; dimension++) {
      const surface = present[dimension] * present[(dimension + 1) % present.length];

      smallestPresentSurface = surface < smallestPresentSurface ? surface : smallestPresentSurface;
