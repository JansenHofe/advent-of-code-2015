...
function calcCookieScore(ingredients, ingredientNumbers) {
  ...
  for (let ingredientIdx = 0; ingredientIdx < ingredients.length; ingredientIdx++) {
    for (let propIdx = 0; propIdx < ingredients[ingredientIdx].properties.length; propIdx++) {
      cookieProperties[propIdx] +=
        ingredientNumbers[ingredientIdx] * ingredients[ingredientIdx].properties[propIdx];
    }
  }
...
function calcCookieCalories(ingredients, ingredientNumbers) {
  let calories = 0;
  for (let ingredientIdx = 0; ingredientIdx < ingredients.length; ingredientIdx++) {
    calories += ingredients[ingredientIdx].calories * ingredientNumbers[ingredientIdx];
  }
  return calories;
}
...
module.exports.getSolution = () => {
  ...
  for (let i = 0; i <= 100; i++) {
    for (let j = 0; j <= 100; j++) {
      for (let k = 0; k <= 100; k++) {
        for (let l = 0; l <= 100; l++) {
          if (i + j + k + l === 100 && calcCookieCalories(ingredients, [i, j, k, l]) === 500) {
